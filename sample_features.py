def sample_features(F_prev,gamma0,data_struct,dist_struct,theta,obsModel):
	...
	feature_inds = np.array([0:Kz_max])
    ...
    log_likelihood = compute_likelihood_unnorm(data_struct[ii],theta,obsModelType,[0:Kz_max],Kz_max,Ks)
    ...
    if np.sum(F[ii,:]) == 0:
	    ...
	    for kk in feature_inds(np.where(featureCounts>0)):
	        ...
	        if featureCounts(kk)>0
	            ...
	            if np.sum(F[ii,:]) == 0:
	                log_likelihood_ii_kk[!Fik_prev+1] = -np.inf
	            else:                
	                pi_init = dist_struct[ii]['pi_init'][F[ii,:]]
	                pi_init = pi_init/np.sum(pi_init)
	                pi_z = dist_struct[ii]['pi_z'][F[ii,:],F[ii,:]]
	                ...
	                if np.isnan(np.sum(neglog_c)):
	                    ...
	                    log_likelihood_ii_kk(!Fik_prev+1) = np.sum(neglog_c) #observation_likelihood(F(ii,:),data_struct(ii),obsModelType,dist_struct(ii),theta);
